...
gulp.task('vendorCSS', function(){
  ...
  return gulp.src(mainBowerFiles(), {base: './bower_components'})
    ...
    .pipe(addSrc.append('./styles/*bootstrap*'))
    .pipe(concat('lib.css'))
    .pipe(gulp.dest('./build/styles'));
});
...
gulp.task('zip-staticresource', function () {
    return gulp.src('**/*', {cwd:path.join(process.cwd(), 'build')})
        .pipe(zip('DGRl.resource'))
        .pipe(gulp.dest('../src/staticresources'));
});

gulp.task('meta-staticresource', function () {
    return createFileFromString('DGRL.resource-meta.xml', '<?xml version="1.0" encoding="UTF-8"?><StaticResource xmlns="http://soap.sforce.com/2006/04/metadata"><cacheControl>Private</cacheControl><contentType>application/zip</contentType></StaticResource>')
        .pipe(gulp.dest('../src/staticresources'));
});
